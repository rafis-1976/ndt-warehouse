...
import {
  ...
  CalendarDays, Barcode, Camera, Plus, CheckCircle2, Trash2,
} from 'lucide-react';
...
export function PrestamoForm({ onSuccess, onCancel }: PrestamoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 1. Insertar un préstamo por cada equipo
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 pt-3 border-t border-gray-100">
          <div className="text-xs text-gray-500 order-2 sm:order-1">
            {equiposSeleccionados.length > 0 && (
              <>
                Se crearán{' '}
                <strong className="text-airbus-blue">
                  {equiposSeleccionados.length}
                </strong>{' '}
                préstamo{equiposSeleccionados.length !== 1 ? 's' : ''} (uno por
                equipo)
              </>
            )}
          </div>
          <div className="flex justify-end gap-3 order-1 sm:order-2">
            ...
            >
              ...
              {equiposSeleccionados.length > 0
                ? `Registrar ${equiposSeleccionados.length} préstamo${
                    equiposSeleccionados.length !== 1 ? 's' : ''
                  }`
                : 'Registrar préstamo'}
            </button>
          </div>
      ...
      {/* ============================================================
          MODAL DE CÁMARA (fuera del form)
          ============================================================ */}
      <Modal
        open={camaraAbierta}
