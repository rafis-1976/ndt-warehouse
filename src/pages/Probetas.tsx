...
import {
  ...
  MapPin, FileText, Box, ScanBarcode,
} from 'lucide-react';
...
export function Probetas() {
  ...
  const handleScan = (code: string) => {
    ...
    // Auto-abrir el detalle si hay match exacto en codigo_barras o pn
    const match = probetas.find(
      (p) =>
        p.codigo_barras?.toLowerCase() === limpio.toLowerCase() ||
        p.pn?.toLowerCase() === limpio.toLowerCase()
    );
    if (match) {
      setTimeout(() => abrirDetalle(match), 150);
    }
  };
    ...
    <div className="p-6 space-y-4">
      ...
      >
        <div className="space-y-3">
          <p className="text-xs text-gray-500">
            Apunta la cámara al código de barras de la probeta. El código se usará como término de búsqueda y, si coincide con una probeta, se abrirá automáticamente.
          </p>
